...
public class SistemaController {
    ...
    public void removerSelecionadas() {
        ...
        List<Medicao> selecionadas = new ArrayList<>();
        int[] linhas = mainFrame.getMedicoesPanel().getTabela().getSelectedRows();
        for (int linha : linhas) {
            int modelRow = mainFrame.getMedicoesPanel().getTabela().convertRowIndexToModel(linha);
            selecionadas.add(tabelaModel.getMedicao(modelRow));
        }
 
        mainFrame.getMedicoesPanel()
                 .removerSelecionadas();
 
        dadosCompletos.removeAll(selecionadas);
